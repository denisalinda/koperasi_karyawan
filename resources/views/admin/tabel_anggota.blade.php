...
<div class="container-fluid">
    ...
    <p class="mb-4">Di halaman ini anda dapat menambahkan data Anggota</a>.</p>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            ...
            <div class="d-flex justify-content-end mb-2 mt-2">
              <div><button type="button" class="btn btn-primary" id="tambah_carosel">Tambah Anggota</button></div>
            </div>
            @endif
     ...
     <div class="modal  fade" id="modal_carosel" tabindex="-1" role="dialog" aria-labelledby="modal_carosel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <h5 class="modal-title" id="exampleModalLongTitle">Anggota</h5>
            ...
            <div class="modal-body">
                ...
                <form id="form_carosel">
                    <div class="mb-3">
                        ...
                        <input type="number" re class="form-control"  {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }} required id="nik" name="nik" aria-describedby="judul">
                      </div>
                    <div class="mb-3">
                        <label for="npwp" class="form-label">NPWP</label>
                        <input type="text" class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }}  id="npwp" name="npwp" aria-describedby="judul">
                    </div>
                    <div class="mb-3">
                        <label for="tanggal_masuk" class="form-label">Tanggal Masuk <a class="red-star">*</a></label>
                        <input type="date" class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }} required id="tanggal_masuk" name="tanggal_masuk" aria-describedby="judul">
                    </div>
                    <div class="mb-3">
                        <label for="nama" class="form-label">Nama <a class="red-star">*</a></label>
                        <input type="text" class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }} required id="nama" name="nama" aria-describedby="judul">
                    </div>
                    <div class="mb-3">
                        <label for="email" class="form-label">Email <a class="red-star">*</a></label>
                        <input type="email" class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }} required id="email" name="email" aria-describedby="judul">
                    </div>
                    <div class="mb-3">
                        <label for="tempat_lahir" class="form-label">Tempat Lahir <a class="red-star">*</a></label>
                        <input type="text" class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }} required id="tempat_lahir" name="tempat_lahir" aria-describedby="judul">
                    </div>
                    <div class="mb-3">
                        <label for="tanggal_lahir" class="form-label">Tanggal Lahir <a class="red-star">*</a></label>
                        <input type="date" class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }}  required id="tanggal_lahir" name="tanggal_lahir" aria-describedby="judul">
                    </div>
                    <div class="mb-3">
                        <label for="jenis_kelamin" class="form-label">Jenis Kelamin <a class="red-star">*</a></label>
                        <select class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }} required name="jenis_kelamin" id="jenis_kelamin">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }}  required id="nomer_telpon" name="nomer_telpon" aria-describedby="judul">
                    </div>
                    <div class="mb-3">
                        <label for="alamat" class="form-label">Alamat <a class="red-star">*</a></label>
                        <textarea name="alamat" class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }}  id="alamat" cols="30" rows="10"></textarea>
                    </div>
                    <div class="mb-3">
                        <label for="foto" class="form-label">Foto</label>
                        <input type="file" class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }}   id="foto" name="foto" aria-describedby="foto">
                    </div>
                    <div class="mb-3">
                        <label for="role" class="form-label">Status <a class="red-star">*</a></label>
                        <div class="form-check">
                            <input class="form-check-input"  {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }} type="radio" value="1" name="status" id="radio_1">
                            <label class="form-check-label" for="radio_1">
                              Aktif
                            </label>
                          </div>
                          <div class="form-check">
                            <input class="form-check-input"  {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }} type="radio" value="0" name="status" id="radio_2" checked>
                    ...
                    @if (auth()->user()->role == "sekretaris")
                        <div class="mb-3">
                            ...
                            <input type="password" class="form-control" {{ auth()->user()->role == "sekretaris"  ? '' :  'disabled' }} id="password" name="password" aria-describedby="judul">
                        </div>
                    @endif
                    @csrf
    ...
    <script>
        $(document).ready(function() {
                 ...
                 $('#form_carosel').submit(function(e){
                     ...
                     if(aksi_status){
                         $.ajax({
                             url: "/admin/tabel-anggota/tambah",
                             method: "POST",
                             data:  data,
                             processData: false,
                             contentType: false,
